...
Autobak_cron_start(){
	crontab -l > "$conf_file/crontab.bak"
	sed -i "/Pearl_OV.sh/d" "$conf_file/crontab.bak"
	echo -e "\n${Crontab_time} bash Pearl_OV.sh autobak" >> "$conf_file/crontab.bak"
	crontab "$conf_file/crontab.bak"
	rm -r "$conf_file/crontab.bak"
	cron_config=$(crontab -l | grep "Pearl_OV.sh")
	if [[ -z ${cron_config} ]]; then
		echo -e "${Error} Автобэкап ${Red}не запущен ${Font_default}" && exit 1
	else
		echo -e "${Info} Автобэкап успешно ${Green}запущен ${Font_default}"
		curl -s -X POST https://api.telegram.org/bot"$bot_api"/sendMessage -d chat_id="$tgid" -d text="Автобэкап сервера успешно запущен"
	fi
}
...
Autobak_cron_stop(){
	...
	if [[ ! -z ${cron_config} ]]; then
		...
		curl -s -X POST https://api.telegram.org/bot"$bot_api"/sendMessage -d chat_id="$tgid" -d text="Автобэкап сервера успешно отключен"
	fi
}
...
Autobak(){
	...
	cd "/etc/"
	tar -czvf "openvpn.tar.gz" "openvpn" && clear
	backupURL="$(curl -H "Max-Downloads: 100" -H "Max-Days: 50" -F filedata=@/etc/openvpn.tar.gz https://transfer.sh)" && clear
	curl -s -X POST https://api.telegram.org/bot"$bot_api"/sendMessage -d chat_id="$tgid" -d text="Бэкап NoProblemVPN %0A Дата: $(date) %0A Ссылка на бэкап: $backupURL "
}
...
${Purple}|3.${Font_default} ${Red} Отправить бэкап принудительно ${Font_default}  ${Purple}|${Font_default}
${Purple}|4.${Font_default} ${Red} Модификация времени автобэкапа${Font_default}  ${Purple}|${Font_default}
...
Set_crontab(){
	echo -e "Введите временный интервал для автобэкапа
 === Описание формата ===
 * * * * * Минуты, часы, дни, месяцы, недели
 ...
 ${Green} 0 2 * * 3 ${Font_default} Каждую среду" && echo
	read -e -p "(По умолчанию: 0 */6 * * * каждые 6 часов):" Crontab_time
	[[ -z "${Crontab_time}" ]] && Crontab_time="0 */6 * * *"
}
